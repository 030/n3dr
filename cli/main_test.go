...
func (n Nexus3) pong() bool {
	...
	if err != nil {
		log.Fatal("Unable to create GET request to nexus's ping endpoint")
	}
	req.SetBasicAuth(n.User, n.Pass)

	resp, err := http.DefaultClient.Do(req)
	if err != nil {
		log.Warn("Request to nexus's ping endpoint failed")
	}
