...
func createNPMRepo() error {
	data := Payload{
		Name:   npmRepositoryName,
		Online: true,
		Storage: Storage{
	...
	req, err := http.NewRequest("POST", os.ExpandEnv(n.Endpoint+"/service/rest/v1/repositories/npm/hosted"), body)
	if err != nil {
		return err
	}
	req.SetBasicAuth("admin", os.ExpandEnv(n.Password))
...
func TestRepositoryRawHTML(t *testing.T) {
	s, _ := n.repositoryRawHTML(n.Endpoint + nexus3BrowserURI + nugetRepositoryName)
...
func TestURLRepoAndDir(t *testing.T) {
	s := urlRepoAndDir("http://localhost:9998/repository/REPO_NAME_HOSTED_NPM/test-one/-/test-one-1.0.0.tgz")
	assert.Equal(t, "REPO_NAME_HOSTED_NPM/test-one", s)
}
...
func TestDownloadURL(t *testing.T) {
	n := Nexus3{Endpoint: "http://localhost:" + strconv.Itoa(d.PortExternal), Repository: npmRepositoryName, BaseDir: nexus3TestDir}
	err := n.AllArtifacts()
	assert.Nil(t, err, ".*")
}

func TestAllArtifactsError(t *testing.T) {
	n.Endpoint = "dsfds"
	n.Repository = nugetRepositoryName
	err := n.AllArtifacts()
	assert.EqualError(t, err, "Get \"dsfds/service/rest/repository/browse/nuget-hosted\": unsupported protocol scheme \"\"")
}
