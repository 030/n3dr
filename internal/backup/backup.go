...
func (n *Nexus3) repositoryDirectoryOrArtifact(errs chan error, url string) error {
	log.Info("Getting all raw HTML...")
	resp, err := n.repositoryRawHTML(url)
	if err != nil {
		return err
	}

	log.Info("Getting all artifacts and repositories from HTML...")
...
func (n *Nexus3) download(url string) (nexusArtifactLabel, error) {
	...
	filePathDir := filepath.Join(n.BaseDir, urlRepoAndDir(url))
...
// downloadAndPrintLabel downloads an artifacts and prints a label to indicate
...
// downloadURL constructs the URL that is used for downloading an artifact
// if browseURL:
//   http://localhost:9998/service/rest/repository/browse/REPO_NAME_HOSTED_NPM/test-two/test-two-1.0.0.tgz
// then the downloadURL should be:
//   http://localhost:9998/repository/REPO_NAME_HOSTED_NPM/test-two/-/test-two-1.0.0.tgz
func downloadURL(url string) string {
	re := regexp.MustCompile("^(.*)/service/rest/repository/browse/(.*)/(.*)$")
	groups := re.FindStringSubmatch(url)
	fqdn := groups[1]
	repositoryNameAndArtifactDirectory := groups[2]
	artifact := groups[3]

	log.Infof("FQDN: '%s', repositoryNameAndArtifactDirectory: '%s', artifact: '%s'", fqdn, repositoryNameAndArtifactDirectory, artifact)
	return fqdn + "/repository/" + repositoryNameAndArtifactDirectory + "/-/" + artifact
}
